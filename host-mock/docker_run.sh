...
docker run --name ${HOST3_NAME} --network "${NETWORK_NAME}" --ip=172.20.0.5 -d -p 6003:5000 -e FLASK_APP=server -e SERVER_NAME=${HOST3_NAME} host-mock
docker run --name ${HOST4_NAME} --network "${NETWORK_NAME}" --ip=172.20.0.6 -d -p 6004:5000 -e FLASK_APP=server -e SERVER_NAME=${HOST4_NAME} host-mock
docker run --name ${HOST5_NAME} --network "${NETWORK_NAME}" --ip=172.20.0.7 -d -p 6005:5000 -e FLASK_APP=server -e SERVER_NAME=${HOST5_NAME} host-mock
